      ...
      <div id='desktop-navbar' className={cn("flex gap-2 justify-between","fixed top-0 left-0 right-0 z-30 transition-colors duration-300 p-4 md:px-6 lg:px-8", 
        ...
        <div className='hidden md:flex gap-2 font-secondary'>
          <nav className='flex gap-6 px-8 items-center'>
            {links.map((link) => (
              <Link key={link.href} href={link.href} className='nav-link opacity-0 mx-2 hover:underline'>
                {link.name}
              </Link>
            ))}
          </nav>
          <Link href={'/#contact'} className={cn('nav-contact', 'opacity-0 shadow-xl p-4 px-6 rounded-full border-2 duration-300', scrolled ? 'bg-just-purple border-just-purple text-white' : 'border-white hover:bg-white text-white hover:text-just-purple')}>
            Contact Me
          </Link>
        </div>
        {/* Mobile menu */}
